...
describe('Integration: /healthz and /dashboard', () => {
  beforeEach(() => {
    vi.spyOn(prisma.transaction, 'findMany').mockResolvedValue(mockTransactions as any);
  });
  ...
  it('should return 400 on /dashboard with invalid params', async () => {
    const response = await request(app).get('/dashboard').query({ type: 'invalid', from: '2026-01-01', to: '2026-01-31' });
